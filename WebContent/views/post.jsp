...
<%@ page import="java.util.*, net.obrien.facebookclone.model.Post, net.obrien.facebookclone.model.User, net.obrien.facebookclone.utils.DataBaseResponse,
 net.obrien.facebookclone.model.Comment" %>

<%@ taglib uri = "http://java.sun.com/jsp/jstl/core" prefix = "c"%>

<%
	@SuppressWarnings("unchecked")
	ArrayList<Post> comments = (ArrayList<Post>) request.getAttribute("comments");
	...
	pageContext.setAttribute("userLikes", userLikes); 
	pageContext.setAttribute("userCommentsLikes", userCommentsLikes);
%>
...
<body>
    <nav class="navbar navbar-expand-lg navbar-light fixed-top">
        ...
        <div class="collapse navbar-collapse" id="navbarSupportedContent">
            ...
            <ul class="navbar-nav">
                <li class="nav-item">
                			<!-- DYNAMICALLY GENERATED PROFILE ROUTE -->
                    <a class="nav-link" href="profile?user_id=<%=session.getAttribute("user_id")%> "> <i class="nav-icons fas fa-user"></i></a>
                </li>
                <li class="nav-item">
    ...
    <main class="profile">
        ...
        <section class="container bottom-profile-section mt-1">
            <div class="row">
                ...
                <section id="post<c:out value="${ post.id}"/>" class="col-12 col-md-8 mx-auto">
                    ...
                    <div class="card  mt-2" style="width: 100%; padding: 1rem">
                        <div class="card-body">
                                    ...
                                    <p class="post-date"> <c:out value="${post.created_at}"/></p>
                                    	...
                                    	<c:if test="${sessionScope.user_id == post.user.id}"> 
				                                <div class="dropdown col-2">
				                                    ...
				                                    <form action="post/delete/post?post_id=<c:out value="${ post.id}"/>" class="dropdown-menu" aria-labelledby="dropdownMenu2">
				                                        <button class="dropdown-item " data-toggle="modal" data-target="#postEditModal" type="button" >Edit</button>
                            ...
                            <div class="post-stat row mt-1">
                                <c:choose>
		                            <c:when test = "${not empty userLikes and userLikes.contains(post.getId())}">
                                ...
                                <div class="col-auto text-muted">
                                    <a class="" data-toggle="collapse" href="#postIdCollapse" role="button"
                        ...
                        <form action="comments/comment?post_id=<c:out value="${post.id}"/> " method="POST">
                ...
                <c:forEach var="comment" items="${comments}">
	                 <section id="comment<c:out value="${comment.id}"/>" class="col-12 col-md-8 mx-auto">
	                    <div class="card  mt-2" style="width: 100%; padding: 1rem">
	                        <div class="card-body">
	                            <!-- POST HEADER -->
	                            <div class="post-header row">
	                                ...
	                                <div class="col-7 mr-0 mr-md-auto">
	                                 <a href="profile?user_id=<c:out value="${comment.user.id}"/>"> 
	                                    <h5 class="user-name"><c:out value="${comment.user.firstname}"/> <c:out value="${comment.user.lastname}"/></h5>
	                                   </a>
	                                    <p class="post-date"> <c:out value="${comment.created_at}"/></p>
	                                </div>
	                                <c:if test="${sessionScope.user_id == comment.user.id}"> 
	                                <div class="dropdown col-2">
	                                    <!-- EDIT DROP DOWN BUTTON -->
	                                    <button class="btn dropdown-toggle" type="button" id="comment<c:out value="${comment.id}"/>"
	                                        data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
	                                        ...
	                                    </button>
	                                    <div class="dropdown-menu" aria-labelledby="dropdownMenu2">
	                                        <button class="dropdown-item " data-toggle="modal"
	                                            data-target="#commentEditModal<c:out value="${comment.id}"/>" type="button">Edit</button>
	                                        <form method="POST" action="comments/delete?comment_id=<c:out value="${comment.id}"/>&post_id=<c:out value="${post.id}"/>"> 
	                            ...
	                            <div class="post-stat row mt-1">
		                            ...
		                            <c:when test = "${comment.liked}">
		                                <form method="POST" action="comments/unlike?comment_id=<c:out value="${ comment.id}"/>" class="col-auto mr-auto text-muted">
		                                <button class="btn dropdown-toggle" type="submit">
		                                    <span> <c:out value="${comment.numberOfLikes}"/>  Likes </span> 
		                                    <span ><i style="color: #1877f2;"class="fas fa-thumbs-up"></i></span>
		                                  </button>
		                                </form>
		                             </c:when>
		                             <c:otherwise>
		                             <form method="POST" action="comments/like?comment_id=<c:out value="${ comment.id}"/>" class="col-auto mr-auto text-muted">
		                                 <button class="btn dropdown-toggle" type="submit">
		                                    <span> <c:out value="${comment.numberOfLikes}"/>  Likes </span> 
		                                    <span ><i class="fas fa-thumbs-up"></i></span>
		                                  </button>
	                ...
	                <c:if test="${sessionScope.user_id == comment.user.id}"> 
					    <!--COMMENT EDIT Modal -->
					    <div class="modal fade" id="commentEditModal<c:out value="${comment.id}"/>" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
					        <div class="modal-dialog modal-dialog-centered">
					            <div class="modal-content">
					                ...
					                <div class="modal-body">
					                    <form method="POST" action="comments/edit?comment_id=<c:out value="${comment.id}"/>" >
					                        <div class="form-group">
					                            <label for="message-text" class="col-form-label">Message:</label>
					                            <textarea name="comment"  class="form-control" id="message-text"><c:out value="${comment.comment}"/></textarea>
					                        </div>
